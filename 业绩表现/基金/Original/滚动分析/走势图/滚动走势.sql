WITH
    ...
    TRADE_DATE_ADJ AS (
        ...
        WHERE L_DATE = L_TRADE_DATE
          AND L_TRADE_DATE BETWEEN TO_CHAR(TO_DATE(${startdate}, 'yyyy-mm-dd'), 'yyyymmdd') AND TO_CHAR(TO_DATE(${enddate}, 'yyyy-mm-dd'), 'yyyymmdd')),
    DATE_INFO AS (
        -- 获取区间交易日内的信息
    ...
    BOND_INDEX_PROFIT AS (
        ...
        WHERE BOND_IDX.L_TRADE_DATE BETWEEN TO_CHAR(TO_DATE(${startdate}, 'yyyy-mm-dd'), 'yyyymmdd')
