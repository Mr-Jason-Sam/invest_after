WITH CONSTANTS AS (SELECT
     ...
     PROVINCE_CONFIG AS (
         -- 到期收益率配置
         SELECT GROUP_CONF.*,
                GROUP_CONF. PVG_MV * GROUP_CONF. PVG_MODIDURA * CONS.ONE_BP PVG_DVBP
         FROM (SELECT SBI.FUND_ID,
                      SBI.TRADE_DATE,
                      SBI.PROVINCE,
                      SUM(SBI.MV)                       PVG_MV,
                      SUM(SBI.PD_POSITION)              PVG_PD_POSI,
                      SUM(SBI.BOND_PFL_POSITION)        PVG_BOND_PFL_POSI,
                      DECODE(SUM(CASE
                      ...
                      DECODE(SUM(CASE
                             ...
                             SUM(CASE
                                     WHEN SBI.BONDTYPE1_WIND IN (CONS.TSF_BOND_NAME, CONS.CHANGE_BOND_NAME) THEN 0
                                     ELSE SBI.MV END))  PVG_YTM

               FROM SELECTOR_BOND_INFO SBI,
                    CONSTANTS CONS
               WHERE SBI.IS_CTZ = CONS.TRUE_CN
               GROUP BY SBI.FUND_ID, SBI.TRADE_DATE, SBI.PROVINCE) GROUP_CONF,
              CONSTANTS CONS)
SELECT *
FROM PROVINCE_CONFIG;