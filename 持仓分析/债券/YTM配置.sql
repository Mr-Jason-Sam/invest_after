WITH CONSTANTS AS (SELECT
     ...
     YTM_CONFIG AS (
         -- 到期收益率配置
         SELECT GROUP_CONF.*,
                GROUP_CONF.YG_MV * GROUP_CONF.YG_MODIDURA * CONS.ONE_BP YG_DVBP
         FROM (SELECT SBI.FUND_ID,
                      SBI.TRADE_DATE,
                      SBI.YTM_GROUP,
                      SUM(SBI.MV)                      YG_MV,
                      SUM(SBI.PD_POSITION)             YG_PD_POSI,
                      SUM(SBI.BOND_PFL_POSITION)       YG_BOND_PFL_POSI,
                      DECODE(SUM(CASE
               ...
               FROM (SELECT SBI.*,
                            CASE
                                WHEN SBI.YTM <= 0.02 THEN '(-∞,2%]'
                                WHEN SBI.YTM <= 0.025 THEN '(2%,2.5%]'
                                WHEN SBI.YTM <= 0.03 THEN '(2.5%,3%]'
                                WHEN SBI.YTM <= 0.035 THEN '(3%,3.5%]'
                                WHEN SBI.YTM <= 0.04 THEN '(3.5%,4%]'
                                WHEN SBI.YTM <= 0.05 THEN '(4%,5%]'
                                WHEN SBI.YTM <= 0.06 THEN '(5%,6%]'
                                WHEN SBI.YTM <= 0.07 THEN '(6%,7%]'
                                WHEN SBI.YTM <= 0.08 THEN '(7%,8%]'
                                ELSE '(8%,+∞]'
                                END YTM_GROUP
                     FROM SELECTOR_BOND_INFO SBI) SBI,
                    CONSTANTS CONS
               GROUP BY SBI.FUND_ID, SBI.TRADE_DATE, SBI.YTM_GROUP) GROUP_CONF,
              CONSTANTS CONS)
SELECT *
FROM YTM_CONFIG;