WITH CONSTANTS AS (SELECT
     ...
     SELECTOR_BOND_INFO AS (
         ...
         FROM (SELECT BOND_INFO.*,
              ...
              CONSTANTS CONS),
     CNBD_RATING_CONFIG AS (
         -- 到期收益率配置
         SELECT GROUP_CONF.*,
                GROUP_CONF.CRG_MV * GROUP_CONF.CRG_MODIDURA * CONS.ONE_BP CRG_DVBP
         FROM (SELECT SBI.FUND_ID,
                      SBI.TRADE_DATE,
                      SBI.CNBD_RATING_GROUP,
                      SUM(SBI.MV)                      CRG_MV,
                      SUM(SBI.PD_POSITION)             CRG_PD_POSI,
                      SUM(SBI.BOND_PFL_POSITION)       CRG_BOND_PFL_POSI,
                      DECODE(SUM(CASE
                      ...
                      DECODE(SUM(CASE
                             ...
                             SUM(CASE
                                     WHEN SBI.BONDTYPE1_WIND IN (CONS.TSF_BOND_NAME, CONS.CHANGE_BOND_NAME) THEN 0
                                     ELSE SBI.MV END)) CRG_YTM

               FROM (SELECT SBI.*,
                            CASE
                                WHEN SBI.RATING_CNBD IN
                                     ('利率债', 'AAA-', 'AA+', 'AA', 'AA(2)', 'AA-', 'A+', 'A', '无评级')
                                    THEN SBI.RATING_CNBD
                                WHEN SBI.RATING_CNBD IN ('AAA+', 'AAA') THEN 'AAA及以上'
                                ELSE 'A以下'
                                END CNBD_RATING_GROUP
                     FROM SELECTOR_BOND_INFO SBI) SBI,
                    CONSTANTS CONS
               GROUP BY SBI.FUND_ID, SBI.TRADE_DATE, SBI.CNBD_RATING_GROUP) GROUP_CONF,
              CONSTANTS CONS)
SELECT *
FROM CNBD_RATING_CONFIG;