WITH CONSTANTS AS (SELECT
     ...
     BOND_INFO AS (
         ...
         FROM (
                  ...
                  SELECT HDB.L_FUND_ID                                                 FUND_ID,
                         ...
                         NVL(BD.VC_COMP_IND1_WIND, '其他')                             IND1_WIND,
                         NVL(BD.VC_COMP_IND2_WIND, '其他')                             IND2_WIND,
                         NVL(BD.VC_COMP_IND3_WIND, '其他')                             IND3_WIND,
                         NVL(BD.VC_COMP_IND4_WIND, '其他')                             IND4_WIND,
                         CASE
                             WHEN BD.VC_BOND_TYPE1_WIND IN ('国债', '央行票据') OR BD.VC_BOND_TYPE2_WIND = '政策银行债'
                                 THEN '利率债'
     ...
     SELECTOR_BOND_INFO AS (
         ...
         FROM (SELECT BOND_INFO.*,
                      DECODE(
                              ${ind_type},
                              CONS.WIND_1, IND1_WIND,
                              CONS.WIND_2, IND2_WIND,
                              CONS.WIND_3, IND3_WIND,
                              CONS.WIND_4, IND4_WIND,
                              IND2_WIND
                          ) IND_TYPE,
                      DECODE(
                              ${modidura_type},
     ...
     IND_TYPE_CONFIG AS (
         -- 到期收益率配置
         SELECT GROUP_CONF.*,
                GROUP_CONF.ITG_MV * GROUP_CONF.ITG_MODIDURA * CONS.ONE_BP ITG_DVBP
         FROM (SELECT SBI.FUND_ID,
                      SBI.TRADE_DATE,
                      SBI.IND_TYPE,
                      SUM(SBI.MV)                      ITG_MV,
                      SUM(SBI.PD_POSITION)             ITG_PD_POSI,
                      SUM(SBI.BOND_PFL_POSITION)       ITG_BOND_PFL_POSI,
                      DECODE(SUM(CASE
                      ...
                      DECODE(SUM(CASE
                             ...
                             SUM(CASE
                                     WHEN SBI.BONDTYPE1_WIND IN (CONS.TSF_BOND_NAME, CONS.CHANGE_BOND_NAME) THEN 0
                                     ELSE SBI.MV END)) ITG_YTM

               FROM SELECTOR_BOND_INFO SBI,
                    CONSTANTS CONS
               GROUP BY SBI.FUND_ID, SBI.TRADE_DATE, SBI.IND_TYPE) GROUP_CONF,
              CONSTANTS CONS)
SELECT *
FROM IND_TYPE_CONFIG;