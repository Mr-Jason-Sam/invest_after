WITH CONSTANTS AS (SELECT
     ...
     YTM_CONFIG AS (
         -- 到期收益率配置
         SELECT GROUP_CONF.*,
                GROUP_CONF.MDG_MV * GROUP_CONF.YG_MODIDURA * CONS.ONE_BP YG_DVBP
         FROM (SELECT SBI.FUND_ID,
                      SBI.TRADE_DATE,
                      SBI.MD_GROUP,
                      SUM(SBI.MV)                      MDG_MV,
                      SUM(SBI.PD_POSITION)             MDG_PD_POSI,
                      SUM(SBI.BOND_PFL_POSITION)       MDG_BOND_PFL_POSI,
                      DECODE(SUM(CASE
                      ...
                      DECODE(SUM(CASE
                             ...
                             SUM(CASE
                                     WHEN SBI.BONDTYPE1_WIND IN (CONS.TSF_BOND_NAME, CONS.CHANGE_BOND_NAME) THEN 0
                                     ELSE SBI.MV END)) YG_YTM

               FROM (SELECT SBI.*,
                            CASE
                                WHEN SBI.MODIDURA <= 0.5 THEN '(-∞,0.5]'
                                WHEN SBI.MODIDURA <= 1 THEN '(0.5,1]'
                                WHEN SBI.MODIDURA <= 2 THEN '(1,2]'
                                WHEN SBI.MODIDURA <= 3 THEN '(2,3]'
                                WHEN SBI.MODIDURA <= 4 THEN '(3,4]'
                                WHEN SBI.MODIDURA <= 5 THEN '(4,5]'
                                WHEN SBI.MODIDURA <= 7 THEN '(5,7]'
                                WHEN SBI.MODIDURA <= 10 THEN '(7,10]'
                                ELSE '(10,+∞]'
                                END MD_GROUP
                     FROM SELECTOR_BOND_INFO SBI) SBI,
                    CONSTANTS CONS
               GROUP BY SBI.FUND_ID, SBI.TRADE_DATE, SBI.MD_GROUP) GROUP_CONF,
              CONSTANTS CONS)
SELECT *
FROM YTM_CONFIG ORDER BY FUND_ID, TRADE_DATE DESC;