WITH CONSTANTS AS (SELECT
     ...
     BOND_TOP_N_INFO AS (
         -- 市值前N大股票
         SELECT SBI.*
         FROM (SELECT SELECTOR_BOND_INFO.*,
                      ROW_NUMBER() OVER (PARTITION BY SELECTOR_BOND_INFO.FUND_ID, SELECTOR_BOND_INFO.TRADE_DATE ORDER BY SELECTOR_BOND_INFO.BOND_PFL_POSITION DESC) PFL_POSI_RANK
               FROM SELECTOR_BOND_INFO) SBI,
              CONSTANTS CONS
         WHERE SBI.PFL_POSI_RANK <= NVL(${bond_top_n}, CONS.DEFAULT_BOND_TOP_N))
SELECT *
FROM BOND_TOP_N_INFO ORDER BY TRADE_DATE DESC, PD_POSITION DESC;