WITH CONSTANTS AS (SELECT
     ...
     SELECTOR_BOND_INFO AS (
         ...
         FROM (SELECT BOND_INFO.*,
              ...
              CONSTANTS CONS),
     ISSUER_RATING_CONFIG AS (
         -- 到期收益率配置
         SELECT GROUP_CONF.*,
                GROUP_CONF. IRG_MV * GROUP_CONF. IRG_MODIDURA * CONS.ONE_BP IRG_DVBP
         FROM (SELECT SBI.FUND_ID,
                      SBI.TRADE_DATE,
                      SBI.ISSUER_RATING_GROUP,
                      SUM(SBI.MV)                       IRG_MV,
                      SUM(SBI.PD_POSITION)              IRG_PD_POSI,
                      SUM(SBI.BOND_PFL_POSITION)        IRG_BOND_PFL_POSI,
                      DECODE(SUM(CASE
                      ...
                      DECODE(SUM(CASE
                             ...
                             SUM(CASE
                                     WHEN SBI.BONDTYPE1_WIND IN (CONS.TSF_BOND_NAME, CONS.CHANGE_BOND_NAME) THEN 0
                                     ELSE SBI.MV END))  IRG_YTM

               FROM (SELECT SBI.*,
                            CASE
                                WHEN SBI.ISSUER_RATING IN ('利率债', 'AAA', 'AAA-', 'AA+', 'AA', 'AA-', 'A+', '无评级')
                                    THEN SBI.ISSUER_RATING
                                ELSE 'A+以下'
                                END ISSUER_RATING_GROUP
                     FROM SELECTOR_BOND_INFO SBI) SBI,
                    CONSTANTS CONS
               GROUP BY SBI.FUND_ID, SBI.TRADE_DATE, SBI.ISSUER_RATING_GROUP) GROUP_CONF,
              CONSTANTS CONS)
SELECT *
FROM ISSUER_RATING_CONFIG;