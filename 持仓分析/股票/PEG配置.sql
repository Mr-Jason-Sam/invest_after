WITH CONSTANTS AS (
    ...
    FROM DUAL),
     ...
     PEG_CONFIG AS (
         -- PEG配置分析
         SELECT PEG_BASE_INFO.L_FUND_ID,
                PEG_BASE_INFO.L_TRADE_DATE,
                PEG_BASE_INFO.PEG_GROUP,
                SUM(STK_MKT)             AS STK_MKT,
                SUM(PD_POSITION) AS PD_POISTION,
                SUM(STK_PFL_POSITION)    AS STK_PFL_POSITION
         FROM (SELECT STK_INFO.*,
                      CASE
                          WHEN STK_INFO.EN_VAL_PEG_EST = 0 THEN '---'
                          WHEN STK_INFO.EN_VAL_PEG_EST <= 0.2 THEN '(0,0.2]'
                          WHEN STK_INFO.EN_VAL_PEG_EST <= 0.4 THEN '(0.2,0.4]'
                          WHEN STK_INFO.EN_VAL_PEG_EST <= 0.6 THEN '(0.4,0.6]'
                          WHEN STK_INFO.EN_VAL_PEG_EST <= 0.8 THEN '(0.6,0.8]'
                          WHEN STK_INFO.EN_VAL_PEG_EST <= 1.1 THEN '(0.8,1.1]'
                          WHEN STK_INFO.EN_VAL_PEG_EST <= 1.5 THEN '(1.1,1.5]'
                          WHEN STK_INFO.EN_VAL_PEG_EST <= 2 THEN '(1.5,2]'
                          WHEN STK_INFO.EN_VAL_PEG_EST <= 5 THEN '(2,5]'
                          WHEN STK_INFO.EN_VAL_PEG_EST <= 10 THEN '(5,10]'
                          ELSE '(10,+∞]'
                          END PEG_GROUP
               FROM STK_INFO) PEG_BASE_INFO
         GROUP BY PEG_BASE_INFO.L_FUND_ID, PEG_BASE_INFO.L_TRADE_DATE, PEG_BASE_INFO.PEG_GROUP)
SELECT *
FROM PEG_CONFIG
ORDER BY L_FUND_ID, L_TRADE_DATE DESC, PEG_GROUP;