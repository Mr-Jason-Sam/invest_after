WITH CONSTANTS AS (
    ...
    FROM DUAL),
     ...
     PB_CONFIG AS (
         -- PB配置分析
         SELECT PB_BASE_INFO.L_FUND_ID,
                PB_BASE_INFO.L_TRADE_DATE,
                PB_BASE_INFO.PB_GROUP,
                SUM(STK_MKT) AS          STK_MKT,
                SUM(PD_POSITION) AS PD_POISTION,
                SUM(STK_PFL_POSITION)    STK_PFL_POSITION
         FROM (SELECT STK_INFO.*,
                      CASE
                          WHEN STK_INFO.EN_VAL_PB = 0 THEN '---'
                          WHEN STK_INFO.EN_VAL_PB <= 1 THEN '(0,1]'
                          WHEN STK_INFO.EN_VAL_PB <= 2 THEN '(1,2]'
                          WHEN STK_INFO.EN_VAL_PB <= 3 THEN '(2,3]'
                          WHEN STK_INFO.EN_VAL_PB <= 4 THEN '(3,4]'
                          WHEN STK_INFO.EN_VAL_PB <= 6 THEN '(4,6]'
                          WHEN STK_INFO.EN_VAL_PB <= 8 THEN '(6,8]'
                          WHEN STK_INFO.EN_VAL_PB <= 10 THEN '(8,10]'
                          WHEN STK_INFO.EN_VAL_PB <= 20 THEN '(10,20]'
                          ELSE '(20,+∞]'
                          END PB_GROUP
               FROM STK_INFO) PB_BASE_INFO
         GROUP BY PB_BASE_INFO.L_FUND_ID, PB_BASE_INFO.L_TRADE_DATE, PB_BASE_INFO.PB_GROUP)
SELECT *
FROM PB_CONFIG
ORDER BY L_FUND_ID, L_TRADE_DATE DESC, PB_GROUP;