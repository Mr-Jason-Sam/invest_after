WITH CONSTANTS AS (
    ...
    FROM DUAL),
     ...
     STK_TOP_INFO AS (
         ...
         SELECT L_FUND_ID,
                L_TRADE_DATE,
                SUM(CASE WHEN MARKET_RANK = 1 THEN STK_PFL_POSITION END)   STK_POS_TOP_1,
                SUM(CASE WHEN MARKET_RANK <= 5 THEN STK_PFL_POSITION END)  STK_POS_TOP_5,
                SUM(CASE WHEN MARKET_RANK <= 10 THEN STK_PFL_POSITION END) STK_POS_TOP_10,
                SUM(CASE WHEN MARKET_RANK <= 20 THEN STK_PFL_POSITION END) STK_POS_TOP_20
         FROM STK_INFO
         GROUP BY L_FUND_ID, L_TRADE_DATE)
SELECT *
FROM STK_TOP_INFO ORDER BY L_FUND_ID, L_TRADE_DATE DESC;