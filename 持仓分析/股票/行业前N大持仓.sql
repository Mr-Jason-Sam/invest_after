WITH CONSTANTS AS (
    ...
    FROM DUAL),
     ...
     INDUSTRY_CONFIG AS (
         -- 行业配置
         SELECT IDS_BASE_CFG.*,
                ROW_NUMBER() over (PARTITION BY L_FUND_ID, L_TRADE_DATE ORDER BY TOTAL_STK_MKT DESC) INDUSTRY_RANK
         FROM (SELECT STK_INFO.L_FUND_ID,
                      STK_INFO.L_TRADE_DATE,
                      STK_INFO.SELECT_INDUSTRY,
                      SUM(STK_INFO.NET_ASSETS_POSITION) TOTAL_NET_AS_POSITION,
                      SUM(STK_INFO.STK_MKT) AS          TOTAL_STK_MKT,
                      SUM(STK_INFO.STK_PFL_POSITION)    TOTAL_POS_STK_PFL
               FROM STK_INFO
               GROUP BY STK_INFO.L_FUND_ID, STK_INFO.L_TRADE_DATE, SELECT_INDUSTRY) IDS_BASE_CFG),
     INDUSTRY_TOP_N_INFO AS (
         -- 市值前N大行业
         SELECT INDUSTRY_CONFIG.*
         FROM INDUSTRY_CONFIG,
              CONSTANTS CONS
         WHERE INDUSTRY_CONFIG.INDUSTRY_RANK <= NVL(${industry_top_n}, CONS.DEFAULT_INDUSTRY_TOP_N))
SELECT *
FROM INDUSTRY_TOP_N_INFO ORDER BY L_FUND_ID, L_TRADE_DATE DESC, INDUSTRY_RANK;